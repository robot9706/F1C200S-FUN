...
static uint8_t vendor[] = "Vendor";
static uint8_t device[] = "USB Mass Storage Device";
static uint8_t serial[] = "000000000000";

static SETUP_PACKET setup;

static uint32_t cbw_tag, cbw_len, cbw_cmd, cbw_addr, bulk_len, bulk_idx;

static union
{
    uint32_t dat[65536 / 4];
...
{
    uint8_t *dsc = ptr;
    uint16_t len = setup.wLength, dlen = dsc[0];
    if (dsc[1] == 2)
        dlen = dsc[2];
    if (dlen < len)
        len = dlen;
...
{
    uint8_t *dsc = ptr;
    uint16_t dlen, len = setup.wLength;
...
{
    // Set PA0 and PA1 to output - no idea what these did
    // PA->DAT = (PA->DAT & ~3) | (i & 3);
...
{
    // CCU->USBPHY_CFG &= ~3; // Clock OFF, Assert reset
    clk_usb_config(0, 1);

    // CCU->BUS_CLK_GATING0 &= ~(1 << 24); // USB clock OFF
    clk_disable(CCU_BUS_CLK_GATE0, 24);
    
    // CCU->BUS_SOFT_RST0 &= ~(1 << 24); // Assert USB OTG Reset
...
void usbd_init(uint32_t numBlocks, usbm_sector_callback read, usbm_sector_callback write)
{
    ...
    // printf("USB: CCU\r\n");
    // CCU->USBPHY_CFG |= 3; // Clock ON, disable reset
    clk_usb_config(1, 0);

    // CCU->BUS_CLK_GATING0 |= (1 << 24); // Enable clock
    clk_enable(CCU_BUS_CLK_GATE0, 24);
