...
void ClearTmp(void)
{
    ...
    if (glob == NULL)
    {
        printf("ClearTmp: Couldn't open dir %s\n", savepathtemp);
        return;
    }
...
void ClearSlot(void)
{
    ...
    if (glob == NULL)
    {
        printf("ClearSlot: Couldn't open dir %s\n", savepath);
        return;
    }
...
void FromCurr(void)
{
    ...
    if (glob == NULL)
    {
        printf("FromCurr: Couldn't open dir %s\n", savepathtemp);
        return;
    }
...
void M_ReadMisObj(void)
{
    ...
    if (f_open(&file, srcpath, FA_OPEN_EXISTING | FA_READ) == FR_OK)
	{
		unsigned int retval = 0;
        f_read(&file, mission_objective, OBJECTIVE_LEN, &retval);
        f_close(&file);
