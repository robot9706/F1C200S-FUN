...
void M_MakeDirectory(const char *path)
{
    ...
    if (res != FR_OK)
    {
        printf("M_MakeDirectory: Failed to create directory %d\n", res);
    }
}
...
boolean M_WriteFile(const char *name, const void *source, int length)
{
    ...
    FRESULT res;
	if ((res = f_open (&file, name, FA_CREATE_ALWAYS | FA_WRITE)) != FR_OK)
	{
		printf ("M_WriteFile: cannot create file %s, %d\n", name, res);
		return false;
	}
