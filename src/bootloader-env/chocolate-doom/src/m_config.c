...
char *M_GetSaveGameDir(const char *iwadname)
{
    ...
    {
        char iwadWithoutExt[256];
        int iwadLen = strlen(iwadname);
        for (int x = 0; x < (iwadLen < 255 ? iwadLen : 255); x++)
        {
            if (iwadname[x] == '.' || iwadname[x] == (char)0 || x == 254)
            {
                iwadWithoutExt[x] = 0;
                break;
            }

            iwadWithoutExt[x] = iwadname[x];
        }

        // Create parts of path
        savegamedir = M_StringJoin(FILES_DIR, NULL);
        M_MakeDirectory(savegamedir);

        savegamedir = M_StringJoin(FILES_DIR, "/savegame/", NULL);
        M_MakeDirectory(savegamedir);

        savegamedir = M_StringJoin(FILES_DIR, "/savegame/", iwadWithoutExt, "/", NULL);
        M_MakeDirectory(savegamedir);
